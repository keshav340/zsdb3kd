...
if args.dataset == 'MNIST' or args.dataset == 'FASHIONMNIST':
    ...
    # Calculate the number of samples for training (10% of the data)
    num_samples = int(0.1 * len(data_train))

    # Modify data loader to use a subset of the training data
    data_train_loader = DataLoader(data_train, batch_size=1024, shuffle=True, num_workers=4,
                                   sampler=torch.utils.data.RandomSampler(data_train, num_samples=num_samples))
...
def train(epoch):
    ...
    # Calculate training accuracy using the number of samples
